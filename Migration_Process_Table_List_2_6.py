...
# Data Validations Include:
# 1. Checksum validation : Yes
# 2. Row count validation : Yes
# 3. Data type validation : No
# 4. Null value validation : No
# 5. Duplicate value validation : No
...
class DatabricksConnector:
    ...
    def move_to_quarantine(self, folder_path, quarantine_container_name):
        """ Move the specified folder to the quarantine area """
        try:
            source_blob_service_client = BlobServiceClient(account_url=self.alpha_storage_url_https, credential=self.alpha_account_key)
            source_container_client = source_blob_service_client.get_container_client(self.alpha_container_name)

            dest_blob_service_client = BlobServiceClient(account_url=self.alpha_storage_url_https, credential=self.alpha_account_key)
            dest_container_client = dest_blob_service_client.get_container_client(quarantine_container_name)

            blobs = source_container_client.list_blobs(name_starts_with=folder_path)
            for blob in blobs:
                source_blob = source_container_client.get_blob_client(blob)
                dest_blob = dest_container_client.get_blob_client(blob)

                # Copy blob to the quarantine container
                dest_blob.start_copy_from_url(source_blob.url)

                # Optionally, delete the blob from the source container
                # source_blob.delete_blob()

            print(f"Moved folder {folder_path} to quarantine")
        except Exception as e:
            print(f"Error moving folder to quarantine: {e}")
            raise e
    ...
    def process_table(self, table_name):
        ...
        if source_df is not None and not source_df.rdd.isEmpty():
            ...
            if self.validate_data(source_df, table_name):
                self.delete_folder_from_blob_storage(table_name)
            else:
                print(f"Data validation failed for table: {table_name}")
                self.move_to_quarantine(blob_path, "quarantine_container_name")
        else:
