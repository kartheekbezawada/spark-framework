...
# Cluster inforamtion is taken to account 
...
if __name__ == "__main__":
    ...
    # Maximum allowed size per batch in MB (80% of 100GB)
    max_batch_size_mb = 80 * 1024  # 80GB in MB
    ...
    for folder in sorted_folders:
        ...
        if current_batch_size + folder_size > max_batch_size_mb:
            # Process the current batch and start a new one
            print(f"Processing batch: {current_batch}")
            databricks_connector.process_all_tables(current_batch)
            current_batch = [folder]
            current_batch_size = folder_size
        else:
            # Add folder to the current batch
